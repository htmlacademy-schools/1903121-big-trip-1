...
const createPhotos = (photos) => `<img class="event__photo" src="${photos.src}" alt="${photos.description}">`;
...
const createTripEditPoint = (event) => {
  ...
  type.arrayType.forEach((typeArray) => {
    if (typeArray.title === type.currentType.title) {
      type.currentType = { ...typeArray, selectedOffers: type.currentType.selectedOffers ? type.currentType.selectedOffers : [] };
    }
  });
  ...
  type.currentType.allOffer.forEach((offer) => {
    ...
    const currentOffer = createOfferForEditAndNewPoint(offer, isCheckedOffer);
    offersView += currentOffer;
  });


  city.arrayCity.forEach((cityArray) => {
    if (cityArray.name === city.currentCity.name) {

      if (city.currentCity.isShowPhoto) {
        city.currentCity = cityArray;
  ...
  if (city.arrayCity) {
    city.arrayCity.forEach((cityElement) => {
      citiesTemplate += `<option value="${cityElement.name}"></option>`;
    });
  }

  if (city.currentCity.isShowPhoto) {
    city.currentCity.pictures.forEach((photo) => {
      const onePhotoTemplate = createPhotos(photo);
  ...
  beginData = dayjs(date.dataBeginEvent).format('YY/MM/DD HH:mm');
  endData = dayjs(date.dataEndEvent).format('YY/MM/DD HH:mm');
  ...
  <form class="event event--edit" action="#" method="post">
    <header class="event__header">
      ...
      <div class="event__field-group  event__field-group--time">
        ...
        <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${beginData}">
        &mdash;
...
export default class EditPointView extends SmartView {
  ...
  #cityChangeHandler = (event) => {
    this.updateData({ city: { currentCity: { name: event.target.value, isShowPhoto: true }, arrayCity: this._data.city.arrayCity } });
  }
